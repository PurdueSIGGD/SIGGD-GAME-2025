...
public class SpiderV3 : MonoBehaviour {
    ...
    [SerializeField] float distToLegCentre = 0.5f; // This controls how much higher than the centre of leg heights it is 
    [SerializeField] LayerMask groundMask; //   Should be set to anything the spider can walk on
    [SerializeField] float maxRaycastDist = 5f; // Half is above the ground target and the other half is below
    [SerializeField] float minDelta = 0.2f; //  Refer to Features section in the summary of this class
    [SerializeField] float maxDelta = 2f; //    Max distance before it takes a step
    [SerializeField] float tiltLerpSpeed = 10f; //   The speed that it lerps the tilt
    ...
    [SerializeField, Range(0, 15)] float bodyPosFrequency = 15f;
    [SerializeField, Range(0, 10)] float bodyPosDamping = 0.75f;
    [SerializeField, Range(-5, 5)] float bodyPosResponse = 0.5f;

    [Header("Leg Settings")]
    [SerializeField] Leg[] leftLegs;
    [SerializeField] Leg[] rightLegs;
    [SerializeField] float legStepTime = 0.25f; //  How long each step takes
    [SerializeField] float legStepHeight = 1f;  //  How high the step should be
    [SerializeField] float timeBeforeRest = 0.5f; //    How long before moving to rest position (refer to minDelta)
    //  These are used for calculating the coefficients for the interpolation used in moving the legs
    [SerializeField, Range(0, 15)] float legFrequency = 15f;
    [SerializeField, Range(0, 10)] float legDamping = 0.75f;
    [SerializeField, Range(-5, 5)] float legResponse = 0.5f;
    ...
    ///     Cyan Lines - the raycast for calculating the locations of the groundTargets
    ///     
    ...
    /// </summary>
    [Header("Debug")]
    [SerializeField] bool showDebugTools;
    [SerializeField] bool onGround; //  If it is grounded (can be improved but im too lazy)
    [SerializeField] int numSteps = 0; //   Number of steps in this group
    [SerializeField] int stepsCompleted = 0; // Number of steps completed by the group
    [SerializeField] bool groupOneMoving; //    If group one is moving or not. Group one is arbitrary up to the setup.
    ...
    void UpdateLegGroup(Leg[] legGroup, int offset) {
        for (int i = 0; i < legGroup.Length; i++) {
            ...
            if (groupOneMoving != (i % 2 == offset) && leg.initialized)
                continue;
    ...
    void UpdateLegPosition(Leg leg, Vector3 pos, Vector3 normal) {
        ...
        if (!leg.initialized) {
            ...
            leg.positionFilter = new SecondOrderDynamics(legFrequency, legDamping, legResponse, leg.leg.target.position);
            leg.upFilter = new SecondOrderDynamics(legFrequency, legDamping, legResponse, leg.leg.target.up);
    ...
    void CalculateBodyPosition() {
        ...
        if (positionFilter == null)
            positionFilter = new SecondOrderDynamics(bodyPosFrequency, bodyPosDamping, bodyPosResponse, targetPos);
