...
namespace ProceduralAnimation.Runtime {
    ...
    /// </remarks>
    public class SpiderLerpController : MonoBehaviour {
        [System.Serializable]
        public class Leg {
        ...
        void CalculateBodyPosition() {
            ...
            RaycastHit hit;
            Physics.Raycast(body.position, -transform.up, out hit, 10, groundMask);

            if (Vector3.Distance(body.position, hit.point) > leftLegs[0].leg.joints[2].length)
                body.localPosition = Vector3.Project(transform.up, hit.normal * (leftLegs[0].leg.joints[2].length + distToLegCentre));
